...
<script >
	export default {
	  ...
	  methods: {
	  	...
	  	operate(operator) {
	  		this.dotSign = false;
	  		if (this.operatorSign === '' && Number.isInteger(parseInt(this.display.slice(-1))) ) {
	  			this.display = `${this.display}${operator}`;
	  			this.operatorSign = operator;
	  		} else if (operator === '-' && (this.operatorSign === '÷' || this.operatorSign === 'x')) {
	  			this.display = `${this.display}${operator}`;
	  			this.operatorSign = `${this.operatorSign}${operator}`;
	  		} else if (this.operatorSign !== operator && this.operatorSign.length !== 2) {
	  			this.display = this.display.slice(0,-1);
	  			this.display = `${this.display}${operator}`;
	  			this.operatorSign = operator;
	  		}
	  	...
	  	equal() {
	  		if (this.operatorSign === '' && (this.display.slice(0,1) !== '÷' && this.display.slice(0,1) !== 'x')) {
	  			const replacedDisplay = this.display.replace(/x/g , "*").replace(/÷/g , "/");
...
<style scoped>
	...
	.btn:focus:not(:active)::after , .btn-animate {
	...
	.calculator{
		...
		border-radius: 10px;
		box-shadow: 0px 15px 20px rgba(0,40,80,.2);
		overflow: hidden;
